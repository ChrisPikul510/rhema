...
exports.default = exports.Attributes = void 0;
const Attributes = ["xmlns", "type", "version", "href", "lang", "source", "target", "platform", "id"];
/**
...
function getValidAttribute(str) {
  ...
  for (const attr of Attributes) {
    if (attr.toLowerCase() == str) return attr;
  }
...
class Node {
  constructor(data) {
    ...
    Object.defineProperty(this, "_customAttrs", {
      configurable: true,
      enumerable: true,
      writable: true,
      value: {}
    ...
    if (data) {
      if (typeof data === 'object' && data.name && data.attributes) this.applyFromSAX(data);
    }
  }
  ...
  getCustomAttributes() {
    return Object.apply({}, this._customAttrs);
  }
  ...
  getAllAttributes() {
    ...
    for (const prop in this) {
      if (this.hasOwnProperty(prop) && prop.startsWith('_') === false) validAttrs[prop] = this[prop];
    }
  ...
  applyFromSAX(node) {
    if (node.attributes && typeof node.attributes === 'object') {
      for (const prop in node.attributes) {
        ...
        if (attr && this.hasOwnProperty(attr)) this.setAttribute(attr, node.attributes[prop].value);else this._customAttrs[node.attributes[prop].name] = node.attributes[prop].value;
      }
    }
  ...
  setParent(node, applyChild = true) {
    ...
    this._parent = nodeId;
    if (applyChild && typeof node !== 'number' && node._id) node.addChild(this, false);
  }
  /**
  ...
  addChild(node, applyParent = true) {
    ...
    if (!exists) {
      ...
      if (applyParent) node.setParent(this, false);
    }
  }
  /**
...
class Rhema {
  constructor() {
    ...
    Object.defineProperty(this, "isProject", {
      configurable: true,
      enumerable: true,
      writable: true,
      value: false
  ...
  parse(data, options = {}) {
    ...
    return new Promise((resolve, reject) => {
      (0, _parser.default)(data).then(obj => {
        if (!obj) {
        ...
        if (obj._tag === 'project') {
          ...
          if (obj.target && obj.target !== '') this.languages.set(obj.target, obj);else if (this.project !== null) {
            const baseEl = this.project.getBaseElement();
            if (baseEl && baseEl.lang == obj.source) this.baseLanguage = obj;
          }
        }
  ...
  export(options = {}) {
    ...
    return new Promise((resolve, reject) => {
      if (!this.loaded || !this.isProject) reject('must have a project loaded');
      ...
      function recursiveAddObject(node) {
        ...
        node._children.forEach(child => {
          if (child._tag == 'group') obj[child.id] = recursiveAddObject(child);else if (child._tag == 'entry') obj[child.id] = child.getTarget() ? child.getTarget().getContent() : child.getSource().getContent();
        });
      ...
      for (const [lng, node] of this.languages.entries()) {
        if (output.hasOwnProperty(lng) === false) output[lng] = recursiveAddObject(node);
      }
...
var _file = _interopRequireDefault(__webpack_require__(/*! ./file */ "./src/node/file.js"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

class Base extends _file.default {
  constructor(...args) {
    var _temp;

    return _temp = super(...args), Object.defineProperty(this, "_tag", {
      configurable: true,
      enumerable: true,
      writable: true,
      value: 'base'
...
var _basenode = _interopRequireDefault(__webpack_require__(/*! ../basenode */ "./src/basenode.js"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

class Entry extends _basenode.default {
  constructor(...args) {
    var _temp;
...
class File extends _basenode.default {
  constructor(...args) {
    var _temp;
...
class Files extends _basenode.default {
  constructor(...args) {
    var _temp;
  ...
  addChild(node, applyParent = true) {
    super.addChild(node, applyParent); //Hook into the addChild to find the file types

    if (node._tag === 'file') this._files.push(node);else if (node._tag === 'base') {
      if (this._base !== null) console.warn('Files element only takes 1 Base element, overriding the old with the new');
      this._base = node;
    }
  }
...
class Group extends _basenode.default {
  constructor(...args) {
    var _temp;
...
var _project = _interopRequireDefault(__webpack_require__(/*! ./project */ "./src/node/project.js"));

var _files = _interopRequireDefault(__webpack_require__(/*! ./files */ "./src/node/files.js"));

var _file = _interopRequireDefault(__webpack_require__(/*! ./file */ "./src/node/file.js"));

var _base = _interopRequireDefault(__webpack_require__(/*! ./base */ "./src/node/base.js"));

var _language = _interopRequireDefault(__webpack_require__(/*! ./language */ "./src/node/language.js"));

var _group = _interopRequireDefault(__webpack_require__(/*! ./group */ "./src/node/group.js"));

var _entry = _interopRequireDefault(__webpack_require__(/*! ./entry */ "./src/node/entry.js"));

var _source = _interopRequireDefault(__webpack_require__(/*! ./source */ "./src/node/source.js"));

var _target = _interopRequireDefault(__webpack_require__(/*! ./target */ "./src/node/target.js"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

/***/ }),

/***/ "./src/node/language.js":
/*!******************************!*\
  !*** ./src/node/language.js ***!
  \******************************/
/*! no static exports found */
...
class Language extends _basenode.default {
  constructor(...args) {
    var _temp;
  ...
  addNode(node) {
    ...
    if (node instanceof _basenode.default) this._nodes.set(node.getID(), node);
  }
...
class Project extends _basenode.default {
  constructor(...args) {
    var _temp;
    ...
    return _temp = super(...args), Object.defineProperty(this, "_tag", {
      ...
      enumerable: true,
      writable: true,
      value: 'rhema.100.project'
    }), Object.defineProperty(this, "_nodeFiles", {
      configurable: true,
      enumerable: true,
      writable: true,
      value: null
  ...
  addNode(node) {
    //We'll keep a dictionary of all the nodes anyways
    if (node instanceof _basenode.default) this._nodes.set(node.getID(), node); //Find some interesting stuff

    if (node._tag === 'files') this._nodeFiles = node;
  }

  hasFiles() {
    return this._nodeFiles && this._nodeFiles._base && this._nodeFiles._files.length > 0;
  }

  countFiles() {
    if (this.hasFiles()) return this._nodeFiles._files.length + 1; //+1 for base file
...
class Source extends _basenode.default {
  constructor(...args) {
    var _temp;
...
class Target extends _basenode.default {
  constructor(...args) {
    var _temp;
...
exports.NodeFromSAX = NodeFromSAX;
exports.ParseRawAttribute = ParseRawAttribute;
exports.default = Parse;

var _sax = _interopRequireDefault(__webpack_require__(/*! sax */ "./node_modules/sax/lib/sax.js"));

var _basenode = _interopRequireDefault(__webpack_require__(/*! ./basenode */ "./src/basenode.js"));

var Nodes = _interopRequireWildcard(__webpack_require__(/*! ./node */ "./src/node/index.js"));
 ...
 */
function NodeFromSAX(node) {
  var outp;

  switch (node.name) {
    case 'project':
      outp = new Nodes.Project();
      break;

    case 'files':
      outp = new Nodes.Files();
      break;
    ...
    case 'language':
      outp = new Nodes.Language();
      break;

    case 'group':
      outp = new Nodes.Group();
      break;

    case 'entry':
      outp = new Nodes.Entry();
      break;

    case 'source':
      outp = new Nodes.Source();
      break;

    case 'target':
      outp = new Nodes.Target();
      break;

    default:
      //Unknown tag types get absorbed as basic Node types
      outp = new _basenode.default();
      outp._tag = node.name;
      break;
  }
...
function ParseRawAttribute(rawStr) {
...
function Parse(data) {
  if (typeof data !== 'string') return Promise.reject("Parse must be supplied with a string of data");
  return new Promise((resolve, reject) => {
    const p = _sax.default.parser(false, {
      trim: true,
      normalize: false,
      lowercase: true,
      xmlns: true,
    ...
    p.onopentag = node => {
      ...
      thisTag = NodeFromSAX(node); //Apply tree-mapping

      if (thisTag) {
        if (!rootDoc && (thisTag instanceof Nodes.Project || thisTag instanceof Nodes.Language)) rootDoc = thisTag;
    ...
    p.ontext = txt => {
      if (lastTag !== null) lastTag.setContent(txt);
    };
