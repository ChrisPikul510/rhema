...
const Attributes = ["xmlns", "type", "version", "href", "lang", "source", "target"];
/**
...
class Node {
  ...
  addChild(node, applyParent = true) {
    ...
    if (!exists) {
      ...
      if (applyParent && typeof node !== 'number' && node._id) node.setParent(this, false);
    }
  }
  /**
    ...
    return new Promise((resolve, reject) => {
      (0, _parser.default)(data).then(obj => {
        ...
        if (obj._tag === 'project') {
          ...
          if (obj.target !== '') this.languages.set(obj.target, obj);else if (this.project !== null) {
...
function NodeFromSAX(node) {
  switch (node.name) {
    case 'project':
      return new Nodes.Project().applyFromSAX(node);

    case 'files':
      return new Nodes.Files().applyFromSAX(node);

    case 'file':
      return new Nodes.File().applyFromSAX(node);

    case 'base':
      return new Nodes.Base().applyFromSAX(node);

    case 'language':
      return new Nodes.Language().applyFromSAX(node);

    case 'entry':
      return new Nodes.Entry().applyFromSAX(node);

    case 'source':
      return new Nodes.Source().applyFromSAX(node);

    case 'target':
      return new Nodes.Target().applyFromSAX(node);

    default:
      //Unknown tag types get absorbed as basic Node types
      const custom = new _basenode.default();
